...
const VoteVerification = () => {
    ...
    const polling_accept = (uid) => {
        setAcceptStart(false);
        axios.post(`http://localhost:4000/VoteAccept/${uid}`).then((response) => {
    ...
    const polling_reject = (uid) => {
        setRejectStart(false);
        axios.post(`http://localhost:4000/VoteReject/${uid}`).then((response) => {
            console.log(response.data);
            alert(response.data.message);
    ...
    const fetchData = () => {
        axios.get(`http://localhost:4000/VerificationPolling/${wid}/${eid}`).then((response) => {
            var data = response.data.VerificationPolling;
            console.log(data);
            setRows(data);
    ...
    return (
        ...
        <Box display="flex" flexDirection="row" marginLeft="20px" my={1} mx={2}>
            ...
            <Paper elevation={3} style={{ padding: '15px', maxWidth: '800', width: '100%', textAlign: 'left' }}>
                ...
                <Typography variant="h4" style={{ fontWeight: 'bold', marginBottom: '16px', textAlign: 'center' }}>Vote Verification</Typography>
            ...
            to={`/Agent/ViewAssign/`}>Back</Button>
                ...
                >
                    <DialogTitle>{"Photo"}
                        <Button onClick={handleClose} style={{ display: 'flex', justifyContent: 'end' }}>back</Button>
                    </DialogTitle>
                    <DialogContent>

                        <img src={photo} height="500px" width="500px" alt='user pic' />
                    </DialogContent>
                    <DialogActions>
                ...
                >
                    ...
                    <DialogActions style={{ padding: '20px', display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Button onClick={() => { polling_accept(id) }}>Accept</Button>
                    </DialogActions>
                </Dialog>
                <Dialog
                    open={openReject}
                ...
                >
                    ...
                    <DialogActions style={{ padding: '20px', display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Button onClick={() => { polling_reject(id) }}>Reject</Button>
                    </DialogActions>
                </Dialog>
            </Paper>
        </Box>
