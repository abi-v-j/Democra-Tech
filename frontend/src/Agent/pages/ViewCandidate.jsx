...
const ViewVoters = () => {
  ...
  const CandidateAccept = (uid) => {
    setAcceptStart(false);
    axios.post(`http://localhost:4000/CandidateAccept/${uid}`).then((response) => {
  ...
  const CandidateReject = (uid) => {
    setRejectStart(false);
    axios.post(`http://localhost:4000/CandidateReject/${uid}`).then((response) => {
      console.log(response.data);
      alert(response.data.message);
      fetchData();
  ...
  const fetchData = () => {
    axios.get("http://localhost:4000/CandidateCheck/"+sessionStorage.getItem("eid")).then((response) => {
      var data = response.data.CandidateCheck;
  ...
  return (
    ...
    <Box display="flex" flexDirection="row" marginLeft="20px" my={1} mx={2}>
      ...
      <Paper elevation={3} style={{ padding: '15px', maxWidth: '800', width: '100%', textAlign: 'left' }}>
        ...
        <Typography variant="h4" style={{ fontWeight: 'bold', marginBottom: '16px', textAlign: 'center' }}>Candidate Verification</Typography>
        ...
        >
          ...
          <DialogActions style={{ padding: '20px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Button onClick={() => { CandidateAccept(id) }}>Accept</Button>
          </DialogActions>
        </Dialog>
        <Dialog
          open={openReject}
        ...
        >
          ...
          <DialogActions style={{ padding: '20px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Button onClick={() => { CandidateReject(id) }}>Reject</Button>
          </DialogActions>
        </Dialog>
      </Paper>
